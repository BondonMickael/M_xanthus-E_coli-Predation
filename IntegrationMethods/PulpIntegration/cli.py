...
def get_parser():
    parser = argparse.ArgumentParser(
        description='Metabolic model integration toolkit'
    )
    ...
    def add_shared_args(p):
        p.add_argument('-f',"--expressionFile", type = str, required=True, 
                       help="Path tothe RNAseq file (.csv or .tsv)")
        p.add_argument('-g', "--geneColName", required=True, type = str, 
                       help="Name of the column with the gene id")
        p.add_argument('-i',"--expressionColName", required=True, type=str,
        ...
        p.add_argument('-a', "--ignore_human", action="store_true", default=True,
    ...
    def add_iMAT_shared_args(p):
        p.add_argument('-e',"--epsilon", type=float, default=1.0, 
                       help='Threshold value to consider a reaction as active, default is 1.')
        p.add_argument('-d', "--discretization", choices=["mean", "quantile"], type=str, default="mean",
                             help="Discretization method (default: mean)")
        p.add_argument('-q',"--quantiles", type=int, nargs=2, default=None,
                             metavar=("qL", "qH"),
                             help=('Quantile thresholds for cutoff of low gene expression (qL) and high gene expression (qH). qL must be lower than qH. Default values are qL=40, qH=70.'))
        p.add_argument('-p', "--optimum", type=float, default=80.0, 
                       help="Minimum optimum percentage of objective reaction (default: 80)")
        p.add_argument('-x', '--oxygenConstraint', type = float, default = None,
                       help = 'Flux value for oxygen exchange reaction EX_o2_e')
    
    #weighted_iMAT
    weighted_imat_parser = subparsers.add_parser("weighted_iMAT", help="Run weighted_iMAT integration. Highly acive reactions are further " \
    ...
    triple_weighted_imat_parser = subparsers.add_parser('triple_weighted_iMAT', help="Run triple_weighted_iMAT integration.")
    ...
    irreversible_imat_fva_parser = subparsers.add_parser("irreversible_iMAT_FVA", help="Run FVA integration using iMAT principles by constraining irreversible highly active reactions to carry flux f +/- epsilon")
    add_shared_args(irreversible_imat_fva_parser)
    add_iMAT_shared_args(irreversible_imat_fva_parser)
    irreversible_imat_fva_parser.add_argument('-z', "--fva_reactionOfInterest", required=True, type=str,
                                 help="Reaction ID of interest for FVA analysis")


    #full iMAT_FVA
    full_imat_fva_parser = subparsers.add_parser("full_iMAT_FVA", help="Run FVA integration using iMAT principles by constraining all highly active reactions to carry flux f +/- epsilon")
    add_shared_args(full_imat_fva_parser)
    add_iMAT_shared_args(full_imat_fva_parser)
    full_imat_fva_parser.add_argument('-z', "--fva_reactionOfInterest", required=True, type=str,
                                 help="Reaction ID of interest for FVA analysis")

    #FVA
    fva_parser = subparsers.add_parser("FVA", help="Run plain Flux Variability Analysis")
    ...
    # FBA
    fba_parser = subparsers.add_parser("FBA", help="Run plain Flux Balance Analysis (no integration)")
