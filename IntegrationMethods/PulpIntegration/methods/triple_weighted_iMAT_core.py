...
def find_leading_expression(rct, gene_expression, ignore_human=False, debbug=False):
    ...
    if gpr_rule is None:
        ...
        raise ValueError(
            f"Reactions {rct.id} has no GPR rule and should therefore not be classified into rH or rL"
        )
    else:
...
def create_expression_variable_full(
    model,
    rH,
    rM,
    rL,
    ...
    weight variables per reaction so that reactions with associated gene expression value. Reactions in RH, if active
    will receive reward c 1<c<2, reactions in RM, if active will receive reward c 0<c<1. Reactions in RL
    do not receive extra weight variable, but can contribute to the objective function with 1 (meaning reaction is inactive)
    ...
    rM: list
        list of moderate active reactions
    rL: list
    ...
    print(
        f"The optimal solution is {objectiveValue}, the model will ask a minimum of {objectiveValue *fraction_of_optimum/100} for the objective reaction."
    )

    prob = pulp.LpProblem("triple_weighted_iMAT_full", pulp.LpMaximize)
    # v_min <= v <= v_max
    v_vars = {}
    ...
    for rid in rH + rL + rM:
        ...
        if rid in (rH + rM):
            ...
            if rid in rH:
                ...
                c = (x - threshold_low) / (threshold_high - threshold_low)
            c_vars[rid] = c
        else:
            prob += lb * (1 - y_f) <= v, f"rL_rM_leftHandSide_{rid}"
            prob += v <= (1 - y_f) * ub, f"rL_rM_rightHandSide_{rid}"

    return prob, v_vars, y_vars, c_vars


def run_tripleWeighted_iMAT(args):
    ...
    # Create PUlp Variables with individual weight for reactions that were classified
    prob, v_vars, y_vars, c_vars = create_expression_variable_full(
        args.model,
        rewardedReactions,
        moderateReactions,
        penalizedReactions,
        ...
        o2_constraint=args.oxygenConstraint,
        ignore_human=args.ignore_human,
    )
    ...
    # Add ATP hydrolysis as objective
    atpObjective = False
    if atpObjective == True:
    ...
    # Output files
        ...
        output_utils.create_outputFiles(
            prob,
            args.output,
            RNASeqData,
            args.discretization,
            ...
            c_vars,
            triple=True,
            o2Constraint=args.oxygenConstraint,
        )
