'''
Module for the core functions of irreversible_iMATlike_FVA. Goal is to run 
regular FVA, but constrain irreversible reactions in set of highly active reactions
...
def run_irreversible_iMAT_FVA(args):
    '''
    ...
    if args.fva_reactionOfInterest not in metabolicModel.reactions:
        raise ValueError(f'Reaction {args.fva_reactionOfInterest} does not exist in given model.')
    RNASeqData = iMAT_utils.generate_RNASeqDataFrame(args.model, args.expressionFile, args.geneColName,
                                          args.expressionColName, args.ignore_human)
    ...
    #Check feasibility
    infeasible_reactions = iMAt_FVA_utils.get_infeasible_reactions(rewardedReactions,metabolicModel,args.epsilon)
    ...
    if len(objective_ids) ==1:
        ...
        raise ValueError(f'Multiple objective reactions cannot be handled fo now, sorry!')
    print(f'The optimal solution is {objectiveValue}, the model will ask a minimum of {objectiveValue *args.optimum/100} for the objective reaction.')

    prob_max = pulp.LpProblem('irreversible_iMAT_FVA_maximize', pulp.LpMaximize)
    prob_min = pulp.LpProblem('irreversible_iMAT_FVA_min', pulp.LpMinimize)
    
    v_vars = {}
    y_var={}
    for rct in metabolicModel.reactions:
        v_vars[rct.id] = pulp.LpVariable(f'v_{rct.id}', lowBound=rct.bounds[0],upBound=rct.bounds[1], cat='Continuous')
