...
def print_output(lpProblem, v_vars, rewardedReactions, penalizedReactions):
    ...
    print(
        f"From {len(rewardedReactions)} highly active reactions, {count1} are active."
    )
    count2 = 0
    for rid in penalizedReactions:
        if fluxes[rid].varValue == 0.0:
            count2 += 1
    print(
        f"From {len(penalizedReactions)} lowly active reactions, {count2} reactions are inactive."
    )
...
def create_outputFiles(
    ...
    if pulp.LpStatus[lpProblem.status] == "Infeasible":
        print(
            f"⚠️ Problem {lpProblem.name} for {expressionColName} is infeasible — skipping."
        )
        if o2Constraint:
    ...
    if not os.path.exists(output_path):
        ...
        if c_vars is None:
            ...
            for rid in v_vars.keys():
                ...
                if rid in y_vars.keys():
                    ...
                    classification = (
                        "high"
                        if rid in rewardedRcts
                        else "moderate" if rid in moderateRcts else "low"
                    )
                    if (
                        (rid in rewardedRcts)
                        or (rid in moderateRcts and triple)
                        or (rid in penalizedRcts and not triple)
                    ):
                        rows.append(
                            {
                                "reaction_id": rid,
        ...
        with open(output_file, "w") as f:
            ...
            if disc_method == "mean":
                ...
                f.write(
                    f"Mean: {dataframe[expressionColName].mean()}\n"
            ...
            f.write(
                f"Solver status:\t{LpStatus[status]}\n"
