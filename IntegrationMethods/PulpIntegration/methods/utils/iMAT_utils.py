...
def generate_RNASeqDataFrame (metabolicModel, expressionFile, geneColName, expressionColName, ignore_human):
    '''
...
def discretization(RNASeq_Df, expressionColName, discretization_method,quantiles=None):
    ...
    '''
    RNASeq_Df['scaled_expression'] = (RNASeq_Df[expressionColName] - RNASeq_Df[expressionColName].min()) / (RNASeq_Df[expressionColName].max() - RNASeq_Df[expressionColName].min())
    ...
    if discretization_method =='mean':
        ...
        upper_threshold_scaled = RNASeq_Df['scaled_expression'].mean() + 1/2*RNASeq_Df['scaled_expression'].std()
        lower_threshold_scaled = RNASeq_Df['scaled_expression'].mean() - 1/2*RNASeq_Df['scaled_expression'].std()        
        ...
        classes = [1,-1]
        RNASeq_Df['discretization']= np.select(classification_rules, classes, default=0)
        ...
        RNASeq_Df.loc[RNASeq_Df[expressionColName] < lower_threshold, 'discretization'] = -1
        RNASeq_Df.loc[RNASeq_Df[expressionColName] > upper_threshold, 'discretization'] = 1

    print(f'Mean: {RNASeq_Df[expressionColName].mean()}\n'
    ...
    return ([lower_threshold, upper_threshold], [lower_threshold_scaled, upper_threshold_scaled],RNASeq_Df)
...
def recursive_evaluation_iMAT(op_list, depth=0, debbug=False):
    ...
    while i < len(op_list):
        ...
        if token == "min":
            ...
            result = max(result, op_list[i + 1]) if result is not None else max(op_list[i - 1], op_list[i + 1])
            i += 2
        else:
