...
def run_single_case(params):
    method, outputpath, exprColName, celltype_name, level, mdl, exprFile, geneColName = params
    if level is None:
    ...
    for c, exprColName in enumerate(exprColNames):
        for i, method in enumerate(methods):
            for level in oxygen_levels:
                params.append((method, outputpaths[i], exprColName, celltype_names[c], level, mdl, exprFile, geneColName))
