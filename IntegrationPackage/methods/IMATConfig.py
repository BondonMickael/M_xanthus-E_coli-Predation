...
class IMATConfig:
    ...
    def __post_init__(self):
        ...
        if self.expression_df.index[0].startswith("MXAN"):
            ...
            raise ValueError(
                "Expression Dataframe seems to not contain mice or human species."
            )
    ...
    def _apply_discretization(self):
        discretizer = Discretizer(
            method=self.discretization_method, quantiles=self.quantiles
        )
        results = discretizer.run(self.expression_df)
        self.expression_df = results.dataframe
        self.lower_threshold_scaled, self.upper_threshold_scaled = (
            results.scaled_thresholds
        )

    def _map_GPR_to_reaction(self):
        self.gpr_mapper = GPRMapper(
            self.metabolicModel, self.expression_df, self.ignore_human
        )
