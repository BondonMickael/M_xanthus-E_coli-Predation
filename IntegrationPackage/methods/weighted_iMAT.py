...
class weighted_iMAT(BasePulpVarConfig):
    ...
    def _create_weight_variables(self):
        ...
        for rid in (self.RH + self.RM):
            ...
            if rid in self.RH: 
                ...
                c = (x - self.lower_threshold_scaled)/(self.upper_threshold_scaled - self.lower_threshold_scaled)
            self.c_vars[rid] = c
    ...
    def _add_objective_function(self):
        terms = (
            [(self.y_vars[rct][1] + self.y_vars[rct][2]) * self.c_vars[rct] for rct in (self.RH + self.RM)] +
