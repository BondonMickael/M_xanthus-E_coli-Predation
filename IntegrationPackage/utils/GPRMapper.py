...
class GPRMapper:
    ...
    def create_reaction_classes(self) -> GPRMapperOutput:
        ...
        '''
        active_genes = self.expression_df.index[self.expression_df['discretization'] == 1].tolist()
        nonActive_genes = self.expression_df.index[self.expression_df['discretization'] == -1].tolist()
        moderateActive_genes = self.expression_df.index[self.expression_df['discretization'] == 0].tolist()
        ...
        for rct in self.metabolicModel.reactions:
            ...
            elif not GPR(gpr_rule).eval(active_genes+moderateActive_genes+nonActive_genes):
                self.RH.append(rct.id)
                
        return GPRMapperOutput(
            RL = self.RL,
            RM = self.RM,
            RH = self.RH
        )
