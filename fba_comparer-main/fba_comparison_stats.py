...
from networkx import edge_betweenness_centrality as betweenness #for color_networks_by_communities including edge weights
...
def example():
    ...
    """
    display(HTML("<u>Example on how this program can be used:</u><br>Here are some examples how to use the different available functions and some suggestions on how to use the parameters.<br><br>\
...
def build_dataframe(model, solutions, conditions):
    ...
    if not(len(solutions) == len(conditions)):
        raise ValueError(f"The number of provided conditions ({len(conditions)}) does not match the number of solutions ({len(solutions)})")
    ...
    for i, cond in enumerate(conditions): # there is probaby a more effective way to do this
        if not (len(solutions[i].fluxes) == len(fba["Reaction"])):
            raise ValueError(f"The condition {cond} has {len(solutions[i].fluxes)} reactions. This does not match the provided model, \
                             which has {len(fba['Reaction'])} reactions.")
        fluxes = []
        for r, (reaction, flux) in zip(fba["ID"], solutions[i].fluxes.items()):
            if not (r == reaction):
                raise ValueError(f"The reation {reaction} in condition {cond} does not match the reaction {r} in list from the model!")
            else:
                fluxes.append(flux)
        fba[cond] = fluxes
    fba["Std_dev"] = fba[conditions].std(axis=1) # calculate the std_dev for each reaction
...
def filter_dataframe(fba, conditions, rounding = False):
    ...
    return fba.loc[(fba[conditions] != 0).any(axis=1)] # remove reactions where all fluxes are zero
...
def normalize_dataframe_cols(fba, conditions, obj_values):
    ...
    if not(len(conditions) == len(obj_values)):
        raise ValueError("The number of conditions does not match the number of objective values")

    normalized_fba = fba.copy()
    for c, v in zip(conditions, obj_values):
        normalized_fba[c] = np.divide(normalized_fba[c], v)
    normalized_fba["Std_dev"] = normalized_fba[conditions].std(axis=1) # calculate new mean and std_dev
...
def normalize_dataframe_rows(fba, conditions, method="div_by_max"):
    ...
    if method == "div_by_sum":
        ...
        if (min < 0):
            shifted -= min 
        normalized = shifted[conditions].div(shifted[conditions].sum(axis=1), axis = 0).mul(100, axis = 0)
        for c in conditions:
    ...
    if method == "minmax":
        ...
        raise ValueError("Invalid mathod. Valid methods are \"minmax\", \"standard\", \"div_by_sum\" and \"div_by_max\"" )
...
def scatter_plot_fluxes(fba, conditions, font_size = 8):
    ...
    if len(conditions) == 2:
        ...
        fig = px.scatter(fba, x = conditions[0], y = conditions[1], hover_name = "ID", hover_data = "Name", color = diff, title = "Scatter plot of reaction fluxes, colored by absolute flux difference")
        fig.update_layout(coloraxis_colorbar_title_text = "diff") 
    else:
        fig = px.scatter_matrix(fba, dimensions=conditions, title = "Pairwise scatter plot of reaction fluxes", hover_name="ID")
...
def bar_plot_flux_variation(fba, conditions, start=0, end=20, ascending=False, color = '#636EFA'):
    """
    ...
    fig = px.bar(sorted_flux[start:end], x='ID', y='Std_dev', hover_data = ['Name'] + conditions, hover_name='ID', color_discrete_sequence=[color])
    return fig
...
""" helper function"""
def _fancy_dendrogram(*args, **kwargs): #adapted from https://joernhees.de/blog/2015/08/26/scipy-hierarchical-clustering-and-dendrogram-tutorial/
...
def hierarchical_clustering_std_dev(fba, start=0, end=0, d_max=0):
    ...
    if (d_max == 0):
        dendrogram(linkage_data, labels=sorted_flux.index, leaf_rotation=90.)
    else:
        _fancy_dendrogram(linkage_data, labels=sorted_flux.index, leaf_rotation=90., annotate_above=10, max_d=d_max,)
    return plt

def pca(fba, conditions, dimensions=2, clusters=1, method="Differences", base_condition="", remove_unchanging = 0.01, abs_values = True, display="with_size"):
    """
    ...
    if not((dimensions == 2) or (dimensions == 3)):
        raise ValueError("Dimensions is only allowed to take on values 2 and 3 (for visualization)")
    if clusters < 1:
        raise ValueError("The number of clusters must be at least 1")
    if not((method=="Correlation") or (method=="Differences")):
        raise ValueError("The parameter method can only be 'Correlation' or 'Differences'")
    ...
    if not (display=="regular" or display=="with_size" or display=="hvplot"):
        raise ValueError(f"The display parameter value {display} is invalid. Valid display values are \"regular\", \"with_size\" and \"hvplot\"")
    if display=="hvplot" and dimensions == 3:
        raise ValueError("For the display type \"hvplot\" only a 2 dimensional version is available.")
    ...
    if method == "Correlation":
        data = flux_coupling_matrix(fba, conditions, remove_unchanging = remove_unchanging, abs_values = abs_values) # create correlation matrix
        ## remove and print all reactions for which the pearson correlation is NaN
        flag = True
        for index, el in data.isna().all(axis=1).items():
            if el == True:
                if flag:
                    print("The following reactions were removed beacuse the pearson correlation was Nan (probably because there was no change in flux)")
                    flag = False
                print(index)
    ...
    pca_df_with_infos = pd.concat([fba_infos[["ID", "Reaction", "Name", "Std_dev"]], pca_df, pd.Series(labels), fba_infos[all_conditions]], axis = 1)
    if (dimensions == 2) :
        pca_df_with_infos.columns = ["ID", "Reaction", "Name", "Std_dev", "PC1", "PC2", "Cluster"] + all_conditions
    else:
        pca_df_with_infos.columns = ["ID", "Reaction", "Name", "Std_dev", "PC1", "PC2", "PC3", "Cluster"] + all_conditions
    ...
    if not (display=="regular"):
        ...
        pca_df_with_infos['Count'] = pca_df_with_infos.groupby(['PC1', 'PC2'])["ID"].transform('count')
        pca_df_with_infos['Node_size'] = [c if c < 40 else 40 for c in pca_df_with_infos['Count']]
    if display=="hvplot":
        pca_df_with_infos['Node_size'] = pca_df_with_infos['Node_size'] * 100
 
    ## create and return plot
    if display == "hvplot":
        return pca_df_with_infos.hvplot.scatter(x='PC1', y='PC2', size='Node_size', by='Cluster', hover_cols=["ID", "Name", "Count", "Std_dev"] + conditions, hover= True,  alpha=0.6,
    ...
    if (dimensions == 2):
        if display == "regular":
            fig = px.scatter(pca_df_with_infos, x='PC1', y='PC2', hover_name='ID', hover_data = ["Name", "Std_dev"] + all_conditions, color="Cluster",
                            title=f'<b>{method} {base_condition}PCA colored by K-means clustering with {clusters} clusters</b>'+f'<br>Total explained variance: {total_var:.2f}%'+f'<br>Explained variance by PC: PC1: {pc1_var:.2f}%, PC2: {pc2_var:.2f}%')
        else: # display = with_size
            fig = px.scatter(pca_df_with_infos, x='PC1', y='PC2', hover_name='ID', hover_data = ["Name", "Count", "Std_dev"] + all_conditions, color="Cluster", opacity=0.5, size='Node_size', size_max=40,
                            title=f'<b>{method} {base_condition}PCA colored by K-means clustering with {clusters} clusters</b>'+f'<br>Total explained variance: {total_var:.2f}%'+f'<br>Explained variance by PC: PC1: {pc1_var:.2f}%, PC2: {pc2_var:.2f}%')
            #fig.update_traces(marker=dict(sizemin=5)) 
    else:
        if display == "regular":
            fig =  px.scatter_3d(pca_df_with_infos, x='PC1', y='PC2', z='PC3', color="Cluster", hover_name='ID', hover_data = ["Name", "Std_dev"] + all_conditions, 
                                title=f'<b>{method} {base_condition}PCA colored by K-means clustering with {clusters} clusters</b>'+f'<br>Total explained variance: {total_var:.2f}%'+f'<br>Explained variance by PC: PC1: {pc1_var:.2f}%, PC2: {pc2_var:.2f}%, PC3: {pc3_var:.2f}%')
        else: # display = with_size
            fig =  px.scatter_3d(pca_df_with_infos, x='PC1', y='PC2', z='PC3', color="Cluster", hover_name='ID', hover_data = ["Name", "Count", "Std_dev"] + all_conditions, opacity=0.5, size='Node_size', size_max=40,
                                title=f'<b>{method} {base_condition}PCA colored by K-means clustering with {clusters} clusters</b>'+f'<br>Total explained variance: {total_var:.2f}%'+f'<br>Explained variance by PC: PC1: {pc1_var:.2f}%, PC2: {pc2_var:.2f}%, PC3: {pc3_var:.2f}%')
    return fig
    
def pca_color_by_pathway(fba, conditions, pathway, pathway_dict, dimensions=2, method="Correlation", base_condition="", remove_unchanging = 0.01, abs_values = True, display="with_size"):
    """
    ...
    # check input
    if not((dimensions == 2) or (dimensions == 3)):
        raise ValueError(f"Dimensions is only allowed to take on values 2 and 3 (for visualization), current value is {dimensions}")
    if not((method=="Correlation") or (method=="Differences")):
        raise ValueError("The parameter method can only be 'Correlation' or 'Differences'")
    if (method == "Differences"):
        if (base_condition == ""):
    ...
    if not(base_condition == ""):
        all_conditions.append(base_condition)
        base_condition = "(to " + base_condition + ") "
    pca_df_with_infos = pd.concat([fba_infos[["ID", "Reaction", "Name", "Std_dev"]], pca_df, pd.Series(mask), fba_infos[all_conditions]], axis = 1, ignore_index=True)
    if (dimensions == 2) :
        pca_df_with_infos.columns = ["ID", "Reaction", "Name", "Std_dev", "PC1", "PC2", "In_pathway"] + all_conditions
    else:
        pca_df_with_infos.columns = ["ID", "Reaction", "Name", "Std_dev", "PC1", "PC2", "PC3", "In_pathway"] + all_conditions
    ...
    ## create and return plot
    if display == "hvplot":
        return pca_df_with_infos.hvplot.scatter(x='PC1', y='PC2', size='Node_size', by='In_pathway', hover_cols=["ID", "Name", "Count", "Std_dev"] + conditions, hover= True,  alpha=0.4,
                                        title=f'{method} {base_condition}PCA colored by presence in pathway {pathway}\n'+f'Total explained variance: {total_var:.2f}%\n'+f'Explained variance by PC: PC1: {pc1_var:.2f}%, PC2: {pc2_var:.2f}%') 
    if (dimensions == 2):
        if display == "regular":
            fig = px.scatter(pca_df_with_infos, x='PC1', y='PC2', hover_name='ID', hover_data = ["Name", "Std_dev"] + all_conditions, color="In_pathway",
                            title=f'<b>{method} {base_condition}PCA colored by presence in pathway {pathway}</b>'+f'<br>Total explained variance: {total_var:.2f}%'+f'<br>Explained variance by PC: PC1: {pc1_var:.2f}%, PC2: {pc2_var:.2f}%')
        else: # display = with_size
            fig = px.scatter(pca_df_with_infos, x='PC1', y='PC2', hover_name='ID', hover_data = ["Name", "Count", "Std_dev"] + all_conditions, color="In_pathway", opacity=0.5, size='Node_size', size_max=40,
                            title=f'<b>{method} {base_condition}PCA colored by presence in pathway {pathway}</b>'+f'<br>Total explained variance: {total_var:.2f}%'+f'<br>Explained variance by PC: PC1: {pc1_var:.2f}%, PC2: {pc2_var:.2f}%')
        fig.update_layout(legend_title = f"In {pathway}")
    else:
        if display == "regular":
            fig =  px.scatter_3d(pca_df_with_infos, x='PC1', y='PC2', z='PC3', color="In_pathway", hover_name='ID', hover_data = ["Name", "Std_dev"] + all_conditions, 
                                title=f'<b>{method} {base_condition}PCA colored by presence in pathway {pathway}</b>'+f'<br>Total explained variance: {total_var:.2f}%'+f'<br>Explained variance by PC: PC1: {pc1_var:.2f}%, PC2: {pc2_var:.2f}%, PC3: {pc3_var:.2f}%')
        else: # display = with_size
            fig =  px.scatter_3d(pca_df_with_infos, x='PC1', y='PC2', z='PC3', color="In_pathway", hover_name='ID', hover_data = ["Name", "Count", "Std_dev"] + all_conditions, opacity=0.5, size='Node_size', size_max=40,
                                title=f'<b>{method} {base_condition}PCA colored by presence in pathway {pathway}</b>'+f'<br>Total explained variance: {total_var:.2f}%'+f'<br>Explained variance by PC: PC1: {pc1_var:.2f}%, PC2: {pc2_var:.2f}%, PC3: {pc3_var:.2f}%')
        fig.update_layout(legend_title = f"In {pathway}")
    return fig

def pca_color_by_list(fba, conditions, reaction_list, dimensions=2, method="Correlation", base_condition="", remove_unchanging = 0.01, abs_values = True, display = "with_size"):
    """
    ...
    if not(base_condition == ""):
        all_conditions.append(base_condition)
        base_condition = "(to " + base_condition + ") "
    pca_df_with_infos = pd.concat([fba_infos[["ID", "Reaction", "Name", "Std_dev"]], pca_df, pd.Series(mask), fba_infos[all_conditions]], axis = 1)
    if (dimensions == 2) :
        pca_df_with_infos.columns = ["ID", "Reaction", "Name", "Std_dev", "PC1", "PC2", "In_list"] + all_conditions
    else:
        pca_df_with_infos.columns = ["ID", "Reaction", "Name", "Std_dev", "PC1", "PC2", "PC3", "In_list"] + all_conditions
    ...
    ## create and return plot
    if display == "hvplot":
        return pca_df_with_infos.hvplot.scatter(x='PC1', y='PC2', size='Node_size', by='In_list', hover_cols=["ID", "Name", "Count", "Std_dev"] + conditions, hover= True,  alpha=0.4,
                                        title=f'{method} {base_condition}PCA colored by presence in reaction list\n'+f'Total explained variance: {total_var:.2f}%\n'+f'Explained variance by PC: PC1: {pc1_var:.2f}%, PC2: {pc2_var:.2f}%') 
    if (dimensions == 2):
        if display == "regular":
            fig = px.scatter(pca_df_with_infos, x='PC1', y='PC2', hover_name='ID', hover_data = ["Name", "Std_dev"] + all_conditions, color="In_list",
                            title=f'<b>{method} {base_condition}PCA colored by presence in reaction list</b>'+f'<br>Total explained variance: {total_var:.2f}%'+f'<br>Explained variance by PC: PC1: {pc1_var:.2f}%, PC2: {pc2_var:.2f}%')
        else: # display = with_size
            fig = px.scatter(pca_df_with_infos, x='PC1', y='PC2', hover_name='ID', hover_data = ["Name", "Count", "Std_dev"] + all_conditions, color="In_list", opacity=0.5, size='Node_size', size_max=40, 
                            title=f'<b>{method} {base_condition}PCA colored by presence in reaction list</b>'+f'<br>Total explained variance: {total_var:.2f}%'+f'<br>Explained variance by PC: PC1: {pc1_var:.2f}%, PC2: {pc2_var:.2f}%')
        fig.update_layout(legend_title = f"in_list")
    else:
        if display == "regular":
            fig =  px.scatter_3d(pca_df_with_infos, x='PC1', y='PC2', z='PC3', color="In_list", hover_name='ID', hover_data = ["Name", "Std_dev"] + all_conditions, 
                                title=f'<b>{method} {base_condition}PCA colored by presence in reaction list</b>'+f'<br>Total explained variance: {total_var:.2f}%'+f'<br>Explained variance by PC: PC1: {pc1_var:.2f}%, PC2: {pc2_var:.2f}%, PC3: {pc3_var:.2f}%')
        else: # display = with_size
            fig =  px.scatter_3d(pca_df_with_infos, x='PC1', y='PC2', z='PC3', color="In_list", hover_name='ID', hover_data = ["Name", "Count", "Std_dev"] + all_conditions, opacity=0.5, size='Node_size', size_max=40,
                                title=f'<b>{method} {base_condition}PCA colored by presence in reaction list</b>'+f'<br>Total explained variance: {total_var:.2f}%'+f'<br>Explained variance by PC: PC1: {pc1_var:.2f}%, PC2: {pc2_var:.2f}%, PC3: {pc3_var:.2f}%')
        fig.update_layout(legend_title = f"in_list")
    return fig
...
def flux_coupling_matrix(fba, conditions, remove_unchanging = 0.01, abs_values = True):
    ...
    """
    unchanging_fba = fba[fba['Std_dev'] >= remove_unchanging] # remove reactions that do not change across conditions
    num_unchanging = len(fba) - len(unchanging_fba)
    if not num_unchanging == 0:
        print(f"{num_unchanging} reaction were removed, because their Std_dev is lower than {remove_unchanging}")

    if abs_values: # get absolute values and remove reactions with swapping signs if the option abs_values is selected
        num_swapping = len(unchanging_fba)
        swapping_fba =  unchanging_fba[~((unchanging_fba[conditions] >= 0).all(axis=1) |  (unchanging_fba[conditions] <= 0).all(axis=1))]
        unchanging_fba = unchanging_fba[(unchanging_fba[conditions] >= 0).all(axis=1) |  (unchanging_fba[conditions] <= 0).all(axis=1)]
        unchanging_fba.loc[:,conditions] = unchanging_fba[conditions].abs().to_numpy()
        num_swapping -= len(unchanging_fba)
        if not num_swapping == 0:
            print(f"The following {num_swapping} reactions were removed because they swap signs across conditions: {list(swapping_fba["ID"])}")
...
def flux_coupling_network(flux_coupling_matrix, pc = 0.8):
    ...
    """
    if pc > 1 or pc < -1:
        raise ValueError(f"Invalid value {pc} for pc, the pc threshold must lie between -1 and 1")
    
    nw = nx.Graph()
    nw.name = f"Flux coupling graph with pc >= {pc}"
    nw.graph['info'] = ""
    nw.add_nodes_from(flux_coupling_matrix.columns, color = "blue")
    rows, cols = np.tril_indices(flux_coupling_matrix.shape[0], k=-1) # get lower triangle of correlation matrix
...
def _most_valuable_edge_with_weights(G): 
    ...
    for edge, btwn in centrality.items(): # divide each betweeness measure by scaled weight to simulate multigraph
        u, v = edge
...
def color_network_by_communities(nw, iterations, include_weights = False):
    ...
    """
    if iterations < 1:
        raise ValueError(f"{iterations} is not a valid value for iterations, the value must be positive")

    # get communities with Girvan_Newman algorithm
    if include_weights: 
        communities_generator = nx.community.girvan_newman(nw, most_valuable_edge=_most_valuable_edge_with_weights)
    else:
    ...
    color_names = px.colors.qualitative.Light24 + list(mcolors.CSS4_COLORS.keys()) # find more colors here: https://plotly.com/python/discrete-color/

    for i, community in enumerate(communities):
        if i > 157:
            raise ValueError("There are too many communities to display (more than 157), try using less iterations of the Girvan Newman algorithm")
...
def visualize_network(nw, layout = "kamada_kawai"):
    ...
    if layout == "kamada_kawai":
        ...
        raise ValueError(f"{layout} is not a valid layout. Valid layouts are kamada_kawai, spring, random and circular.")

    node_colors = [nw.nodes[node]['color'] for node in nw.nodes()]

    plt.figure(figsize=(10, 10))
    nx.draw(nw, pos, node_size=25, edge_color='gray', width=0.5, node_color=node_colors, alpha=0.5, with_labels=True, font_size=7)
...
def visualize_interactive_network(nw, fba, conditions, layout = "kamada_kawai", edge_weights = True):
    """
    ...
    node_info = [f"{fba["ID"][i]}<br>{fba["Name"][i]}{_flux_info(fba, i, conditions)}<br>Std_dev: {fba["Std_dev"][i]:.3f}" for i in index]
    ...
    for edge in nw.edges(data=True):  
        ...
        if edge_weights:
            scaled_weight = edge[2].get("weight", 1) ** 5 # scale edge thickness by the correlation coefficient
            edge_width = max(0.1, float(scaled_weight))  # ensure minimum thickness

        fig.add_trace(go.Scatter(x=[x0, x1, None],  # separate edges with None
            y=[y0, y1, None], mode='lines', line=dict(width=edge_width, color="gray")))

    # add nodes (this part is adapted from ChatGPT)
    node_x, node_y = zip(*[pos[node] for node in nw.nodes()])
    fig.add_trace(go.Scatter(x=node_x, y=node_y, mode='markers+text', marker=dict(size=9, color=node_colors, opacity=0.5), #or use size 15 for bigger nodes, normal size is 9
        text=list(nw.nodes), textposition="middle center", hovertext=node_info, hoverinfo="text"))

    fig.update_layout(title=dict(text=nw.name + nw.graph["info"], x=0.5), title_font_size=18,showlegend=False, hovermode='closest', width=1000, height=1000,
        plot_bgcolor='white', xaxis=dict(visible=False), yaxis=dict(visible=False), margin=dict(l=100, r=100, t=70, b=0), font=dict(size=10)) #or use size 20 (or 12) for a bigger font size, normal size is 10 
    return fig
...
def get_community_list(nw):
    ...
    for index, (key, val) in enumerate(community_map.items(), start=1):
        ...
        if len(val) > 10:
            dots = "..."
        legend_text.append(f"{index}. ({key}) with {len(val)} elements: {val[0:9]}{dots}")
    legend_color = list(community_map.keys())

    fig = go.Figure() # create empty dummy figure (bc I don' know how else to create a legend without a figure)
    for text, color in zip(legend_text, legend_color):
        fig.add_trace(go.Scatter(x=[None], y=[None], mode='markers', marker=dict(size=10, color=color), name=text))
    fig.update_layout(showlegend=True, xaxis=dict(visible=False), yaxis=dict(visible=False), legend=dict(x=0.5, y=0.5, xanchor="center", yanchor="middle",orientation="v"),
                        width=1000, height=300, plot_bgcolor='white', margin=dict(l=0, r=0, t=0, b=0))
...
def community_bar_plots(fba, conditions, com_list):
    ...
    flux_dist = pd.DataFrame({"Condition": conditions}) # dataframe with just the flux values
    ...
    fig = make_subplots(rows=rows, cols=num_cols, subplot_titles=[str(i) + f" with {len(com_list[c])} reactions" for i, c in enumerate(com_list.keys(), start = 1)])

    for i, com in enumerate(com_list.keys()):
        row = (i // num_cols) + 1  # calculate row index
        col = (i % num_cols) + 1  # calculate column index
        fig.add_trace(go.Bar(x=flux_dist["Condition"], y=flux_dist[com], marker=dict(color=com), opacity = 0.5), row=row, col=col)

    fig.update_layout(title="Flux mean per condition of each community ", height=rows * 200, width=1000, showlegend=False )
    return fig
...
def community_box_plots(fba, conditions, com_list):
    ...
    fig = make_subplots(rows=num_rows, cols=num_cols, subplot_titles=["community " + str(i) + f" with {len(com_list[c])} reactions" for i, c in enumerate(com_list.keys(), start = 1)])

    for i, com in enumerate(com_list.keys()): # create sub plots
        row = i // num_cols + 1
        col = i % num_cols + 1
        indices = [fba[fba["ID"] == id].index[0] for id in com_list[com]] # create df with flux values for each community
        ...
        for c in conditions: # add box for each condition
            fig.add_trace(go.Box(y=com_df[c], name=c, boxmean=True, marker=dict(color=com), opacity = 0.6), row=row, col=col)

    fig.update_layout(title="Flux value distribution for the different communities", height=200 * num_rows, width=1000, showlegend=False)
    return fig

def community_pw_summary(com_list, id_dict, pw_dict, lower_bound = 5, upper_bound = 100, cut_off = 0.1):
    """
...
def community_sankey_diagram(com_list, pw_list, fba, id_dict, diagram_height = 500, pw_hierarchy = {}, unchanging = 0.01, text_size = 14):
    """
    ...
    if pw_hierarchy:
        for pathway in pw_list:
            if not pathway in pw_hierarchy.keys():
                raise ValueError(f"The pw_list must be a subset of the pw_hierarchy. The entry {pathway} was not found in the pw_hierarchy.")
    ...
    unchanging_list = fba[fba['Std_dev'] <= unchanging]["ID"] # identify pathways corresponding to unchanging reactions
    ...
    for c, (com, reactions) in enumerate(com_list.items(), start = len(pw_list) + 1): # identify pathways correspnding tpo communities
        ...
        for pw, reactions in enumerate(community_pws):
            if len(reactions) > 0:
                ...
                targets.append(c)
                values.append(len(reactions))
                colors.append(f"rgba{mcolors.to_rgba(com, opacity)}") # make colors more transparent
                r_labels = ""
                for r, reaction in enumerate(reactions):
                    if r == 0:
                        r_labels += f"{reaction}"
                        ...
                        r_labels += f", {reaction}"
                labels.append(f"reactions: {r_labels}") # add reaction list to hoverdata

    # create plot
    fig = go.Figure(data=[go.Sankey(valueformat = ".0f", valuesuffix = " reactions",
                                    node = dict(pad = 15, thickness = 20, line = dict(color = "black", width = 0.2), label = pw_list + ["unchanging reactions"] + [str(i) for i in range(1,len(com_list.keys()) + 1)],
    ...
    fig.update_layout(title_text="Sankey Diagram of pathway distribution across communities", font_size=text_size, height = diagram_height)
    return fig


if __name__ == "__main__":
    print("Currently only intended as import into jupyter notebooks.\nThe following functions are available:\n \
          - build_dataframe(model, solutions, conditions)\n \
          - scatter_plot_fluxes(fba, conditions)\n \
          - correlation_matrix(fba, conditions[opt:, color])\n \
          - filter_dataframe(fba, conditions)\n \
